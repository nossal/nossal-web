                  ...
                  (seq [[:head
                        ...
                        [:body.coupon
                         [:section
                          ...
                          [:div.intro.text
                           [:p (cdata :text)]]
                          (a-out (cdata :url) {:id "get-coupon" :data-vars-coupon service :class (str "code" service)} (cdata :code))
                         ...
                         [:section.others
                          ...
                          [:p (map (fn [x]
                                     [:a {:href (str "/cupons/" x)} "Cupom " (first ((coupon-codes x) :title)) " " [:span (rest ((coupon-codes x) :title))]])
                                   (keep #(if (not= service %) %) (shuffle (keys coupon-codes))))]]
                         [:footer
